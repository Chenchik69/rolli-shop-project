...
window.addEventListener('click', function(event) {
    ...
    if (event.target.hasAttribute('data-cart')) {
        ...
        // Собираемданные с этого товара и записываем их в объект productInfo
        const productInfo = {
            id: card.dataset.id,
        ...
        // Если товар есть в корзине или же его там нет
        if (itemInCart) {
            const counterElement = itemInCart.querySelector('[data-counter]');
            counterElement.innerText = parseInt(counterElement.innerText) + parseInt(productInfo.counter);
        } else {
        // Собранные данные подставим в шаблон для товара с корзины
        const cartItemHTML = `
